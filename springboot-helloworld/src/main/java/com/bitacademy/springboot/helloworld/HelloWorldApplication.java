...
public class HelloWorldApplication {
	public static void main(String[] args) {
		 ...
		 *  2. 만약, 웹 어플리케이션이면, 웹 어플리케이션 타입을 결정(Spring MVC or Reactive)
		 *  3. Annotation Scanning(Auto) + Configuration Class(Explicity)를 통한 Bean 생성/등록/와이어링 작업
		 *  4. 만약, 웹 어플리케이션이고 타입이 Spring MVC이면 
		 *  	- 내장(embeded) 서버(TomcatEmbededServiceServletContainer) 인스턴스 생성
		 *  	- 서버 인스턴스에 웹 어플리케이션을 배포
		 *  	- 서버 인스턴스 실행
		 *  5. ApplicationRunner 인터페이스를 구현한 Bean을 Application Context에서 찾아서 실행(run 호출)
		 */
